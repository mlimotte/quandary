...
[ ] 2. bd sync                 (commit beads changes)
[ ] 3. jj commit -m "..."      (commit code)
[ ] 4. bd sync                 (commit any new beads changes)
[ ] 5. jj git push             (push to remote)
```

**NEVER skip this.** Work is not done until pushed.
**NOTE**: Use `jj` for all version control—never bare `git` commands.
...
- Jj workflow: hooks auto-sync, run `bd sync` at session end
...
### Sync & Collaboration
- `bd sync` - Sync with jj remote (run at session end)
